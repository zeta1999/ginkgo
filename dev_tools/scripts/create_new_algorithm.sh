...
function list_sources {
    for type in solver preconditioner matrix factorization reorder
    do
...
NAME=${name^^}
    ...
    if [[ "$name" == "" ]] || ( [[ "$source_type" != "preconditioner" ]] && [[ "$source_type" != "matrix" ]] && [[ "$source_type" != "solver" ]] && [[ "$source_type" != "factorization" ]] && [[ "$source_type" != "reorder" ]]) || [[ "$source_name" == "" ]]; then
    print_help
    exit 1
fi
